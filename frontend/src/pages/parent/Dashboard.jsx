...
export default function ParentDashboard() {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 flex items-center justify-center p-6">
        <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-8 border border-red-200 max-w-md w-full text-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 space-y-6 pb-8">
      ...
      <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-8 border border-white/20">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-gray-600 text-lg mt-1">Theo dõi chuyến đi của các con hôm nay</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-blue-100 hover:shadow-2xl hover:scale-105 transition-all duration-300">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm font-medium flex items-center gap-2">
                <span>👨‍👩‍👧‍👦</span> Tổng số con
              </p>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-green-100 hover:shadow-2xl hover:scale-105 transition-all duration-300">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm font-medium flex items-center gap-2">
                <span>🚌</span> Đang trên xe
              </p>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-yellow-100 hover:shadow-2xl hover:scale-105 transition-all duration-300">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-gray-600 text-sm font-medium flex items-center gap-2">
                <span>⏰</span> Chờ đón
      ...
      <div className="space-y-4">
        ...
        {children.length === 0 ? (
          <div className="bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl p-12 text-center border border-white/20">
            ...
            <p className="text-gray-600 text-lg font-semibold">Chưa có thông tin học sinh</p>
            <p className="text-gray-400 text-sm mt-2">Vui lòng liên hệ nhà trường để đăng ký</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {children.map((child) => (
              <div
                key={child.id}
                className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden border border-white/20 hover:scale-[1.02]"
              >
                  ...
                  <div className="relative flex items-center justify-between">
                    ...
                    {child.busStatus?.trip && (
                      <div className={`px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 backdrop-blur-sm border ${
                        ...
                        {getStatusText(child.busStatus.trip.status)}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-100">
                      <div className="space-y-2.5">
                        ...
                        {child.busStatus.driver && (
                          <p className="text-sm text-gray-700 ml-10 flex items-center gap-1">
                            <span>👨‍✈️</span> Tài xế: <span className="font-semibold">{child.busStatus.driver.name}</span>
                          </p>
                        )}
                        {child.busStatus.trip?.picked_at && (
                          <p className="text-xs text-gray-600 ml-10 flex items-center gap-1">
                            <span>⏰</span> Lên xe lúc: <span className="font-medium">{new Date(child.busStatus.trip.picked_at).toLocaleTimeString('vi-VN')}</span>
                          </p>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button
                      ...
                      className="flex-1 bg-gradient-to-r from-gray-100 to-gray-200 hover:from-gray-200 hover:to-gray-300 text-gray-700 font-bold py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-md hover:shadow-lg hover:scale-105"
                    >
