...
import { getScheduleStudents, updateTripStatus, updateDriverLocation } from '../../api/trips';
...
const DriverMap = () => {
  ...
  const maybeSendLocation = async (loc) => {
    try {
      const now = Date.now();
      // throttle to every 5s to avoid spamming server
      if (now - lastUpdateSentRef.current < 5000) return;
      lastUpdateSentRef.current = now;
      await updateDriverLocation(loc.lat, loc.lng, 0, 0, loc.accuracy || 10);
    } catch (e) {
      // silent fail to avoid disturbing driver UI
      console.debug('updateDriverLocation failed (ignored):', e?.message || e);
    }
  };
  ...
  const getCurrentLocation = () => {
    if (!navigator.geolocation) {
      ...
      // do not send default fake location to server
  setAlert({ type: 'info', message: t(lang, 'using_default_location_hcm') });
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        // persist once on initial acquire
        maybeSendLocation(location);
  ...
  setAlert({ type: 'error', message: t(lang, 'device_no_gps') });
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        setCurrentLocation(location);
        updateMapMarkers(location);
        // periodically send to backend while tracking
        maybeSendLocation(location);
      },
      (error) => {
